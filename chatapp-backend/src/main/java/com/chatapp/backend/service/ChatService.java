...
public class ChatService {
            ...
            .ifPresent(recipient -> {
                ...
                if (online) {
                    ...
                    if (token != null && !token.isBlank()) {
                        // Send FCM notification — image messages show "📷 Photo" as preview
                            String preview = message.getType() == Message.MessageType.IMAGE
                                ? "📷 Photo"
                                : message.getContent();
                            log.info("Sending FCM text message notification to offline user {}", recipient.getId());
                            fcmService.sendTextMessageNotification(
                                token,
                                chat.getId(),
                                senderName,
                                preview,
    ...
    public MessagePayload toPayload(Message message, Chat chat) {
        ...
        return MessagePayload.builder()
                ...
                .thumbnailBase64(message.getThumbnailBase64())
